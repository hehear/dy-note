...
(function($) {
  /**
   * A super class of config logics for all search services
  ...
  HexoSearch = function(options) {
          ...
          if (foundMatch) {
            ...
            if (first_occur >= 0) {
              ...
              keywords.forEach(function(keyword) {
                ...
                match_content = match_content.replace(regS, "<b style='color: red'>"+keyword+"</b>");
              });
    ...
    self.buildResultList = function(data, queryText) {
      ...
      $.each(data, function(index, post) {
          ...
          post.permalink="/article/"+post._source.id+".html";
          post.title=post._source.article_name;
          post.digest=post._source.article_short;
          post.text=post._source.article_short;
    ...
    self.query = function(queryText, startIndex, callback) {
      if (!self.cache) {
        ...
        if(self.cache.lastQuery==queryText){
            //console.log("查询内容相同走缓存******")
            var results = "";
...
(function($) {
    ...
    /**
     * Search by HehearSearch generator json content
     * @param options : (object)
     */
    HehearSearch = function(options) {
        SearchService.apply(this, arguments);
        var self = this;
        self.config.endpoint = ROOT + ((options||{}).endpoint || "content.json");
        self.config.endpoint = self.config.endpoint.replace("//","/"); //make sure the url is correct
        ...
        self.buildResultList = function(data, queryText) {
            ...
            $.each(data, function(index, post) {
                ...
                post.permalink="/article/"+post.ID+".html";
                post.title=post.ARTICLE_NAME;
                post.digest=post.ARTICLE_SHORT;
                post.text=post.ARTICLE_SHORT;
                if (self.contentSearch(post, queryText))
        ...
        self.query = function(queryText, startIndex, callback) {
            if (!self.cache) {
                ...
                $.ajax({
                    url: '/rest/article/queryArticleContent',
                    type: 'post',
                    data: {query:queryText},
                    success: function (rs) {
